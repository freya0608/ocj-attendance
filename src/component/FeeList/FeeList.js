...
export default function FeeList() {
    ...
    const [fee,setFeeList] = useState({feeList:[]});
    useEffect(()=>{
        axios.get('/getFeeList',{params:{page:0}}).then(res=>{
            // console.log(res)
            setFeeList(oldValues => ({
                ...oldValues,
                feeList:res.data.msg.rows,
                count:res.data.msg.count
            }));
        })
    },[]);
    console.log('fee',fee)
    const emptyRows = rowsPerPage - Math.min(rowsPerPage, fee.count - page * rowsPerPage);

    function handleChangePage(event, newPage) {
        setPage(newPage);
        axios.get('/getFeeList',{params:{page:newPage}}).then(res=>{
            // console.log(res)
            setFeeList(oldValues => ({
                ...oldValues,
                feeList:res.data.msg.rows,
        ...
        <Paper className={classes.root}>
            <div className={classes.tableWrapper}>
                <Table className={classes.table}>
                    ...
                    <TableBody>
                        {fee&&fee.feeList&&fee.feeList.map(row => (
                    ...
                    <TableFooter>
                        <TableRow>
                            <TablePagination
                                ...
                                page={page}
                                SelectProps={{
                                    inputProps: { 'aria-label': 'fee.feeList per page' },
                                    native: true,
                                }}
