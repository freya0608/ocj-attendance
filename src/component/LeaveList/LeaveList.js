...
export default function LeaveList() {
    ...
    const [leave,setLeaveList] = useState({leaveList:[]});
    useEffect(()=>{
        axios.get('/getLeaveList',{params:{page:0}}).then(res=>{
            // console.log(res)
            setLeaveList(oldValues => ({
                ...oldValues,
                leaveList:res.data.msg.rows,
    ...
    const emptyRows = rowsPerPage - Math.min(rowsPerPage, leave.count - page * rowsPerPage);

    function handleChangePage(event, newPage) {
        console.log('newPage',newPage)
        setPage(newPage);
        axios.get('/getLeaveList',{params:{page:newPage}}).then(res=>{
            // console.log(res)
            setLeaveList(oldValues => ({
                ...oldValues,
                leaveList:res.data.msg.rows,
                ...
                <Table className={classes.table}>
                    <TableHead>
                        <TableRow>
                            ...
                            <StyledTableCell align="center">请假原因</StyledTableCell>
                            <StyledTableCell align="center">通过</StyledTableCell>
                        </TableRow>
                    </TableHead>
                    <TableBody>
                        {/*.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)*/}
                        {leave&&leave.leaveList&&leave.leaveList.map(row => (
                            <TableRow key={row.name}>
                                ...
                                <TableCell align="center">{row.reason||'无'}</TableCell>
                    ...
                    <TableFooter>
                        <TableRow>
                            <TablePagination
                                ...
                                page={page}
                                SelectProps={{
                                    inputProps: { 'aria-label': 'leave.leaveList per page' },
                                    native: true,
                                }}
